...
% regularization
regConst = lambda / (2 * m) * (sum(sum(Theta1(:, 2:end) .^2)) + sum(sum(Theta2(:, 2:end) .^2)));
J = 1 / m * sum(sum((-y_matrix .* log(a3) - (1 - y_matrix) .* log(1 - a3)))) + regConst;

% Error between hypothesis and actual value
delta3 = a3 - y_matrix;
